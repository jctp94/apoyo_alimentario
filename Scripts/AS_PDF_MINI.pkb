CREATE OR REPLACE package body as_pdf_mini
is
--
  type tp_objects_tab is table of number( 10 )
    index by pls_integer;

  type tp_pages_tab is table of blob
    index by pls_integer;

  type tp_char_width_tab is table of pls_integer
    index by pls_integer;
  ...
  type tp_font_tab is table of tp_font
    index by pls_integer;

  type tp_pls_tab is table of pls_integer
    index by pls_integer;
...
--
  ...
  is
    ...
    begin
      ...
      loop
        t_rv( i ) :=
          utl_raw.cast_to_binary_integer( utl_raw.substr( t_tmp
                                                        , i * 4 + 1
                                                        , 4
                                                        ) );
      end loop;
...
--
  ...
  is
  begin
    return rtrim( rtrim( to_char( p_value
                                , rpad( '9999999990D'
                                      , 11 + p_precision
                                      , '0'
                                      )
...
--
  ...
  begin
    ...
    loop
      s1 :=
        mod(   s1
             + utl_raw.cast_to_binary_integer( dbms_lob.substr( p_src
                                                              , 1
                                                              , i
                                                              ) )
           , 65521
...
--
  begin
    ...
    dbms_lob.append( t_blob
                   , hextoraw( adler32( p_val ) )
                   );
...
--
  ...
  is
  begin
    return add_object2pdfdoc(    '/Type/Font'
                              || '/Subtype/'
...
--
  ...
  begin
    ...
    then
      ...
      add2pdfdoc(    '/ColorSpace [/Indexed /DeviceRGB '
...
--
  ...
  begin
    ...
    add2pdfdoc(    '/MediaBox [0 0 '
                || to_char_round( settings.page_width
                                , 0
                                )
                || ' '
                || to_char_round( settings.page_height
                                , 0
                                )
                || ']' );
    add2pdfdoc( '>>' );
...
--
  ...
  is
  begin
    return add_object2pdfdoc(    '/Type/Catalog'
                              || '/Pages '
                              || to_char( add_pages )
                              || ' 0 R'
...
--
  ...
  begin
    ...
    loop
      add2pdfdoc( to_char( objects_tab( i )
                         , 'fm0000000000'
                         ) || ' 00000 n' );
...
--
  ...
  begin
    ...
    dbms_lob.createtemporary( pages_tab( pages_tab.count( ) - 1 )
                            , true
                            );
--
    ...
    settings.y :=
      ...
      - settings.margin_top
      - nvl( settings.current_fontsizept
           , 12
           );
...
--
    ...
    then
      add2page(    'BT /F'
                || settings.current_font
                || ' '
                || to_char_round( settings.current_fontsizept )
                || ' Tf ET' );
    end if;
  end;
...
--
  ...
  begin
    if rawtohex( dbms_lob.substr( p_img_blob
                                , 8
                                , 1
                                ) ) != '89504E470D0A1A0A'
    ...
    dbms_lob.createtemporary( t_img.pixels
                            , true
                            );
    ind := 9;

    loop
      len := raw2num( dbms_lob.substr( p_img_blob
                                     , 4
                                     , ind
                                     ) );                            -- length
      exit when len is null or ind > dbms_lob.getlength( p_img_blob );

      case utl_raw.cast_to_varchar2
                                  ( dbms_lob.substr( p_img_blob
                                                   , 4
                                                   , ind + 4
                                                   ) )           -- Chunk type
        when 'IHDR'
        then
          t_img.width := raw2num( dbms_lob.substr( p_img_blob
                                                 , 4
                                                 , ind + 8
                                                 ) );
          t_img.height :=
                         raw2num( dbms_lob.substr( p_img_blob
                                                 , 4
                                                 , ind + 12
                                                 ) );
          t_img.color_res :=
          ...
          color_type := raw2num( dbms_lob.substr( p_img_blob
                                                , 1
                                                , ind + 17
                                                ) );
        ...
        when 'PLTE'
        then
          t_img.color_tab := dbms_lob.substr( p_img_blob
                                            , len
                                            , ind + 8
                                            );
        when 'IDAT'
        then
          dbms_lob.append( t_img.pixels
                         , dbms_lob.substr( p_img_blob
                                          , len
                                          , ind + 8
                                          )
...
--
  ...
  is
    ...
    t_ind integer;
  begin
    if (    dbms_lob.substr( p_img_blob
                           , 2
                           , 1
                           ) != hextoraw( 'FFD8' )       -- SOI Start of Image
         or dbms_lob.substr( p_img_blob
                           , 2
                           , dbms_lob.getlength( p_img_blob ) - 1
                           ) != hextoraw( 'FFD9' )         -- EOI End of Image
       )
...
--
    ...
    if dbms_lob.substr( t_img.pixels
                      , 2
                      , 3
                      ) in
...
--
      t_ind := 3;
      t_ind :=
         t_ind + 2 + raw2num( dbms_lob.substr( t_img.pixels
                                             , 2
                                             , t_ind + 2
                                             ) );

      loop
        buf := dbms_lob.substr( t_img.pixels
                              , 2
                              , t_ind
                              );
        exit when buf = hextoraw( 'FFDA' );               -- SOS Start of Scan
        exit when buf = hextoraw( 'FFD9' );                -- EOI End Of Image
        exit when substr( rawtohex( buf )
                        , 1
                        , 2
                        ) != 'FF';
        ...
        then
          ...
          then
            t_img.color_res :=
                     raw2num( dbms_lob.substr( t_img.pixels
                                             , 1
                                             , t_ind + 4
                                             ) );
            t_img.height :=
...
--
  ...
  begin
    if t_type is null
    then
      if rawtohex( dbms_lob.substr( p_blob
                                  , 8
                                  , 1
                                  ) ) = '89504E470D0A1A0A'
      then
...
--
    img :=
      case lower( t_type )
        when 'png'
          then parse_png( p_blob )
        when 'jpg'
          then parse_jpg( p_blob )
      end;
...
--
  ...
  begin
    ...
    dbms_lob.createtemporary( pdf_doc
                            , true
                            );
    settings := null;
    objects_tab.delete;
...
--
  ...
  is
    t_fh sys.utl_file.file_type;
    t_len pls_integer := 32767;
  begin
    t_fh := sys.utl_file.fopen( p_dir
                          , p_filename
                          , 'wb'
                          );
    finish_pdf;
    ...
    loop
      sys.utl_file.put_raw( t_fh
                      , dbms_lob.substr( pdf_doc
                                       , t_len
                                       , i * t_len + 1
                                       )
                      );
...
--
  ...
  begin
    return case lower( p_unit )
      when 'mm'
        then p_value * 72 / 25.4
      when 'cm'
        then p_value * 72 / 2.54
      when 'pt'
        then p_value                                            -- also point
      when 'point'
        then p_value
      when 'inch'
        then p_value * 72
      when 'in'
        then p_value * 72                                        -- also inch
      when 'pica'
        then p_value * 12
      when 'p'
        then p_value * 12                                        -- also pica
      when 'pc'
        then p_value * 12                                        -- also pica
      when 'em'
        then p_value * 12                                        -- also pica
      when 'px'
        then p_value                       -- pixel voorlopig op point zetten
      when 'px'
        then p_value * 0.8                                           -- pixel
      else null
    end;
  end;
...
--
  ...
  begin
    case upper( p_format )
      when 'A3'
      then
        settings.page_height := conv2user_units( 420
                                               , 'mm'
                                               );
        settings.page_width := conv2user_units( 297
                                              , 'mm'
                                              );
      when 'A4'
      then
        settings.page_height := conv2user_units( 297
                                               , 'mm'
                                               );
        settings.page_width := conv2user_units( 210
                                              , 'mm'
                                              );
      when 'A5'
      then
        settings.page_height := conv2user_units( 210
                                               , 'mm'
                                               );
        settings.page_width := conv2user_units( 148
                                              , 'mm'
                                              );
      when 'A6'
      then
        settings.page_height := conv2user_units( 148
                                               , 'mm'
                                               );
        settings.page_width := conv2user_units( 105
                                              , 'mm'
                                              );
      when 'LEGAL'
      then
        settings.page_height := conv2user_units( 356
                                               , 'mm'
                                               );
        settings.page_width := conv2user_units( 216
                                              , 'mm'
                                              );
      when 'LETTER'
      then
        settings.page_height := conv2user_units( 279
                                               , 'mm'
                                               );
        settings.page_width := conv2user_units( 216
                                              , 'mm'
...
--
  ...
  is
  begin
    settings.page_width := conv2user_units( p_width
                                          , p_unit
                                          );
    settings.page_height := conv2user_units( p_height
                                           , p_unit
                                           );
  end;
...
--
  ...
  is
  begin
    settings.margin_left := conv2user_units( p_left
                                           , p_unit
                                           );
    settings.margin_right := conv2user_units( p_right
                                            , p_unit
                                            );
    settings.margin_top := conv2user_units( p_top
                                          , p_unit
                                          );
    settings.margin_bottom := conv2user_units( p_bottom
                                             , p_unit
                                             );
  end;
...
--
  ...
  begin
    if ( p_family is null and p_style is null and p_fontsizept is null )
    then
      return;
    end if;

    t_style :=
      replace( replace( replace( replace( replace( upper( p_style )
                                                 , 'NORMAL'
                                                 , 'N'
                                                 )
                                        , 'REGULAR'
    ...
    t_style :=
      nvl( t_style
         , case
             when settings.current_font is null
               then 'N'
             else fonts( settings.current_font ).style
           end
         );
    t_family :=
      nvl( lower( p_family )
         , case
             when settings.current_font is null
               then 'helvetica'
             else fonts( settings.current_font ).family
           end
         );
    ...
    loop
      ...
      then
        ...
        settings.current_fontsizept :=
                    coalesce( p_fontsizept
                            , settings.current_fontsizept
                            , 12
                            );
        settings.encoding :=
          nvl( utl_i18n.map_charset( p_encoding
...
--
    ...
    loop
      t_char :=
             utl_raw.cast_to_binary_integer( dbms_lob.substr( t_tmp
                                                            , 1
                                                            , i
                                                            ) );
      t_width :=
...
--
  ...
  is
    ...
    t_line_height number := nvl( p_line_height
                               , settings.current_fontsizept
                               );
    t_start number := nvl( p_start
...
--
    ...
    then
      ...
      while(     instrc( p_txt
             ...
             and string_width( substrc( p_txt
                                      , 1
                                      , instrc( p_txt
                                              , ' '
                                              , 1
      ...
      if instr( p_alignment
              , 'right'
              ) > 0 or instr( p_alignment
                            , 'end'
                            ) > 0
      then
        t_x := t_start + t_width - t_len;
      elsif instr( p_alignment
                 , 'center'
                 ) > 0
      then
...
--
  function rgb(
    p_hex_rgb in varchar2
  )
    return varchar2
  is
  begin
    return    to_char_round
...
--
  ...
  is
    t_use_color boolean;
  begin
    add2page( 'q ' || to_char_round( p_line_width, 5 ) || ' w' );
...
--
  ...
  begin
    ...
    if substr( p_line_color
             , -6
             ) != substr( p_fill_color
                        , -6
                        )
    then
      add2page( to_char_round( p_line_width
                             , 5
                             ) || ' w' );
    end if;
    ...
    add2page(    to_char_round( p_x
              ...
              || ' '
              || to_char_round( p_width
                              , 5
                              )
              || ' '
              || to_char_round( p_height
                              , 5
                              )
              || ' re '
              || case
...
--
    ...
    then
      ...
      add2page(    'q '
                || to_char_round( nvl( p_width
                                     , images( t_ind ).width
                                     ) )
                || ' 0 0 '
                || to_char_round( nvl( p_height
                                     , images( t_ind ).height
                                     ) )
                || ' '
                || to_char_round( case
                                    when p_x > 0
                                      then p_x
                                    else -p_x - images( t_ind ).width / 2
                                  end )
                || ' '
                || to_char_round( case
                                    when p_y > 0
                                      then p_y
                                    else -p_y + images( t_ind ).height / 2
                                  end )
                || ' '
                || ' cm /I'
                || to_char( t_ind )
